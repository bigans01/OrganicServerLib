									   ...
									   EnclaveFractureResultsMap* in_fractureResultsMapRef)
{
	...
	for (auto& currentForgedPolySetIter : planPolyRegistry.polySetRegistry)
	{
		...
		// 3. The produced EnclaveTriangles in each ORE of steps #1 and #2 are used in the call to appendContourPlanEnclaveTriangleSkeletons; at this point,
		...
		ContouredPlanUtils::appendContourPlanEnclaveTriangleSkeletons(blueprintKey, &planMassManager.contouredPlanMass, in_ecbMapRef, oreTrackerRef);		// 4.) for each blueprint in the adherence list, 
	}

	// DEBUG: let's get the state of a specific Ore, if it exists...
	planMassManager.checkStateOfSpecificOre(EnclaveKeyDef::EnclaveKey(0, -1, -1), EnclaveKeyDef::EnclaveKey(1, 7, 7));

	EnclaveKeyDef::EnclaveKey debugBlueprintKey(0, -1, -1);
	EnclaveKeyDef::EnclaveKey debugOREKey(1, 7, 7);

	std::cout << "(ContouredMountain::runMassDrivers) -> checking contents of actual persistent OREs. " << std::endl;
	auto permECBMapBPFinder = in_ecbMapRef->checkIfBlueprintExists(debugBlueprintKey);
	if (permECBMapBPFinder == true)
	{
		std::cout << "(ContouredMountain::runMassDrivers): found Blueprint of ORE to check, at key: ";
		debugBlueprintKey.printKey();
		std::cout << std::endl;

		auto permOREFinder = in_ecbMapRef->checkIfBlueprintContainsSpecificOre(debugBlueprintKey, debugOREKey);
		if (permOREFinder == true)
		{
			std::cout << "(ContouredMountain::runMassDrivers): found key of ORE to check, at key: ";
			debugOREKey.printKey();
			std::cout << std::endl;

			ORELodState currentLodState = in_ecbMapRef->getOrganicRawEnclaveRef(debugBlueprintKey, debugOREKey)->getLodState();
			std::string lodStatePrefix = "(BlueprintMassManager::checkStateOfSpecificOre): LOD state of ORE is: ";
		...
		{
			std::cout << "(ContouredMountain::runMassDrivers): Found blueprint, but not ORE to check. " << std::endl;
		}
	}
	else
	{
		std::cout << "(ContouredMountain::runMassDrivers): Didn't find the blueprint to check. " << std::endl;
	}

	std::cout << "Done with debug for plan mass manager." << std::endl;
	int doneWithMassManagerDebug = 3;
	std::cin >> doneWithMassManagerDebug;
